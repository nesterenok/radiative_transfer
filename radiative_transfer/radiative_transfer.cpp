...
void calc_h2o_para_masers(string input_data_path, string sim_data_path, string output_path, int verbosity, bool is_shock_data_used);
void calc_h2o_ortho_masers(string input_data_path, string sim_data_path, string output_path, int verbosity);
...
#pragma omp master 
    ...
    lin_out.clear();
    lin_out.str("");
    lin_out << "output_2e4_3-15.txt";  // check the file name
    ...
    int i, j, k, verbosity(1);
    double v;
    string path, suff, suff1, suff2, suff3, sim_data_path, input_data_path, output_path;
...
//    ch3oh_coll_extrapolations(input_data_path, output_path = "");
  ...
  //sim_data_path = "C:/Users/Александр/Documents/Данные и графики/paper Cosmic masers in C-type shocks/output_data_2e4_magnf-b2/shock_cr1-15_25/";
    sim_data_path = "C:/Users/Александр/Documents/Данные и графики/paper Cosmic masers in C-type shocks/output_data_2e5/shock_cr1-16_15/";
 
    // check - extrapolated or not data on CH3OH collisions
    output_path = "C:/Users/Александр/Documents/Данные и графики/paper Cosmic masers in C-type shocks/ch3oh_2e5/shock_cr1-16_15/";
...
//    calc_oh_masers_test(input_data_path, sim_data_path, output_path, verbosity);
    ...
    verbosity = false;
#pragma omp parallel shared(input_data_path, sim_data_path, save_line_stat, verbosity), private(i, output_path)
    {
        double time_in, time_tot;
        stringstream sstr;
...
#pragma omp for schedule (dynamic, 1)
        for (k = 0; k <= 40; k += 1) 
        {
            ...
            output_path += suff;
            calc_oh_masers(input_data_path, sim_data_path, output_path, save_line_stat = false, verbosity);  // no line statistics

            // no line overlap
            output_path = path + "oh_nolo_";
            output_path += suff;
            // calc_oh_masers_test(input_data_path, output_path, output_path, verbosity);  
            
            // check - extrapolated or not data on CH3OH collisions
            output_path = path + "ch3oh_";
            output_path += suff;
            calc_ch3oh_a_masers(input_data_path, sim_data_path, output_path, save_lev_pop = false, verbosity);
            calc_ch3oh_e_masers(input_data_path, sim_data_path, output_path, verbosity);
            
/*            
            output_path = path + "h2o_";
            output_path += suff;
            calc_h2o_para_masers(input_data_path, sim_data_path, output_path, verbosity, is_shock_data_used = false);
            calc_h2o_ortho_masers(input_data_path, sim_data_path, output_path, verbosity);
...
void save_mol_vibr_data(std::string fname, cloud_data* cloud, energy_diagram* mol_levels, double* lev_pop, int nb_lev)
{
    ...
    for (i = 0; i < cloud->nb_lay; i++) {
        ...
        // vibration state number densities are saving [cm-3]
