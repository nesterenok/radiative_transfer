	...
	if (!outfile.is_open()) 
		...
		while (it != data.end())
		{
			outfile << left 
				...
				<< setw(15) << setprecision(4) << it->inv << setw(15) << it->gain << setw(15) << it->lum << setw(15) << it->tau_eff 
				<< setw(13) << it->exc_temp_arr[fixed_nb] << setw(13) << setprecision(2) << it->tau_sat << endl;
			it++;
		}
		outfile.close();
	}
